...
カスタムメトリクスの入れ物部分が不要か判断  
コメントをわかりやすくする  
サンプルのままではなく、可視化したとき分かりやすくする  
  
分かった事  
カスタムメトリック・ディスクリプタは作成しなくても、グローバルで自動的にメトリクスが作成される  
タイムシリーズでデータポイントを作成するときはリソースは、線の名前を付けれてくれるだけなので、今回は使わない  
メトリクス名で一意の名前を作成すれば、スタックドライバーからメトリクス名で検索可能で、簡単にグラフが作れる  
メトリクス名やプロジェクト名の外部変数化は、メトリクスやプロジェクトが複数無いので、実装しない  
